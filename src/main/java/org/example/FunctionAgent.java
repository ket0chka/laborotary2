...
public class FunctionAgent extends Agent {
    ...
    private static final Logger logger = Logger.getLogger(FunctionAgent.class.getName());
    ...
    private class InitiatorBehaviour extends Behaviour {
        ...
        public void action() {
            ...
            ACLMessage reply = myAgent.blockingReceive(mt, 10000); // Блокируем выполнение до получения ответа с таймаутом 10 секунд
            if (reply != null) {
                // Обработка ответа
                ...
                logger.info("Updated x: " + x + ", delta: " + delta);
                ...
                block(); // Блокируем выполнение, если ответ не получен
            }
        }
